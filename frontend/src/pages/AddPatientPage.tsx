...
export default function AddPatientPage({ onCreate, selectedPatient, ocrLanguage, setNotice, onDocumentSaved }: Props) {
  ...
  const filteredDoctorOptions = useMemo(() => {
    ...
    return Array.from(names).sort((a, b) => a.localeCompare(b));
  }, [appointment.department, doctorScheduleOptions]);
  ...
  const handleScheduleAppointmentToken = async () => {
    ...
    if (!appointment.doctor) {
      setNotice({ type: "error", message: "Please select a doctor for the appointment." });
      return;
    }
    if (!paymentConfirmed) {
      setNotice({ type: "error", message: "Please verify and confirm that the consultation fee has been paid by checking the 'Verify Fee Paid' checkbox." });
      return;
    }
    ...
    try {
      ...
      setSavingAppointment(true); // Wait: actually setting to false after save
    ...
    <section className="form-layout patient-registration-module">
      <div className="panel patient-registration-card">
        ...
        <form id={registrationFormId} className="grid-form patient-grid-form exact-patient-form" onSubmit={handleSubmit}>
          ...
          <Label>Primary Mobile<div style={{ display: "flex", gap: "8px" }}><Select value={primaryCountryCode} onChange={(event) => setPrimaryCountryCode(event.target.value)} style={{ maxWidth: "130px" }}>{COUNTRY_CODES.map((code) => <option key={code} value={code}>{COUNTRY_CODE_RULES[code].label}</option>)}</Select><Input value={form.phone} onChange={handleChange("phone")} placeholder="Mobile number" inputMode="numeric" required maxLength={(COUNTRY_CODE_RULES[primaryCountryCode] || COUNTRY_CODE_RULES["+91"]).max} /></div>{validationErrors.phone && <span className="field-error">{validationErrors.phone}</span>}</Label>
          ...
          <Label>Address / Area<Textarea value={form.address} onChange={handleChange("address")} required rows={2} placeholder="Enter complete address" />{validationErrors.address && <span className="field-error">{validationErrors.address}</span>}</Label>
